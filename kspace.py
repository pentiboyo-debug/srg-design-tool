...
def dual_input(label, min_val, max_val, default_val, step, k, fmt=None, sidebar=False, container=st):
    ...
    container.markdown(f"<div style='font-size:11px; margin-top:5px;'>{label}</div>", unsafe_allow_html=True)
    col1, col2 = container.columns([7, 3])
...
def dual_range_input(label, min_val, max_val, default_val, step, k, container=st):
    ...
    container.markdown(f"<div style='font-size:11px; margin-top:5px;'>{label}</div>", unsafe_allow_html=True)
    c1, c2, c3 = container.columns([5.4, 2.3, 2.3])
...
# --- 5. Sidebar Layout UI inside st.form (Prevents Auto-Rerun on Every Click) ---
st.sidebar.markdown("### 💾 Configuration Management")
col_s1, col_s2 = st.sidebar.columns(2)
with col_s1: st.download_button("Save Setup", data=export_settings(), file_name="waveguide_config.json", use_container_width=True)
with col_s2: 
...
# [핵심 개정] 사이드바 입력 컴포넌트 전체를 st.form 구조 내부로 캡슐화 처리
with st.sidebar.form(key="waveguide_parameter_form"):
    path_choice = st.radio("Grating Path Type", ["Path A (ICG→OC)", "Path B (ICG→EPE→OC)"], index=1, horizontal=True, key="path_choice")
    single_layer_sync = st.checkbox("Single Layer Mode (RGB Sync)", value=True, key="single_layer_sync")
    coord_sys = st.radio("K-Space Coordinates System", ["Absolute Wavevector (nm⁻¹)", "Normalized Wavevector (Direction Cosine)"], index=1, horizontal=True, key="coord_sys")

    st.markdown("---")
    st.markdown("**⚙️ Light Engine Alignment Tilt Panel**")
    le_tilt_x = dual_input("LE Horizontal Incident Angle θ_x (°)", -30.0, 30.0, 0.0, 0.1, "le_tilt_x", "%.1f", container=st.sidebar)
    le_tilt_y = dual_input("LE Vertical Incident Angle θ_y (°)", -30.0, 30.0, 0.0, 0.1, "le_tilt_y", "%.1f", container=st.sidebar)

    st.markdown("---")
    st.markdown("**📐 Hardware Glass Properties**")
    n_d_in = dual_input("Substrate Index (n at 589nm)", 1.0, 3.0, 1.75, 0.01, "n_d", "%.2f", container=st.sidebar)
    abbe_v_in = dual_input("Abbe Number (Vd)", 10.0, 100.0, 35.0, 0.1, "abbe_v", "%.1f", container=st.sidebar)
    thickness_in = dual_input("Substrate Thickness t (mm)", 0.1, 3.0, 0.40, 0.01, "thickness", "%.2f", container=st.sidebar) 
    epd_val_in = dual_input("Light Engine EPD Size (mm)", 1.0, 50.0, 3.5, 0.1, "epd_val", "%.1f", container=st.sidebar) 
    h_fov = dual_range_input("Horizontal FOV Bounds (°)", -60, 60, (-30, 30), 0.01, "h_fov", container=st.sidebar)
    v_fov = dual_range_input("Vertical FOV Bounds (°)", -60, 60, (-20, 20), 0.01, "v_fov", container=st.sidebar)
    m_ord = st.selectbox("Diffraction Order (m)", [1, -1, 2, -2], index=1, key="m_order_select")

    st.markdown("---")
    st.markdown("**📐 Out-Coupler Aperture Form Factor**")
    oc_width = dual_input("OC Aperture Width (mm)", 5.0, 100.0, 30.0, 0.1, "oc_width", "%.1f", container=st.sidebar)
    oc_height = dual_input("OC Aperture Height (mm)", 5.0, 100.0, 20.0, 0.1, "oc_height", "%.1f", container=st.sidebar)

    st.markdown("---")
    st.markdown("**🔮 Grating Vector Rotation Alignment**")
    angle_icg = dual_input("ICG Vector Angle (°)", 0.0, 360.0, 0.0, 0.01, "angle_icg", "%.2f", container=st.sidebar)
    angle_epe = dual_input("EPE Vector Angle (°)", 0.0, 360.0, 240.0, 0.01, "angle_epe", "%.2f", container=st.sidebar) if "Path B" in path_choice else 0.0

    auto_oc_angle = st.checkbox("Auto-find OC Angle (Zero-tilt Target)", value=False, key="auto_oc_angle")
    if auto_oc_angle:
        st.caption("🔒 OC Angle is locked in optimized auto tracking mode.")
        angle_oc = 120.0 
    else:
        angle_oc = dual_input("OC Vector Angle (°)", 0.0, 360.0, 120.0, 0.01, "angle_oc", "%.2f", container=st.sidebar)

    st.markdown("---")
    def get_wl_inputs_form(name, def_l, def_p, n_d, V_d):
        act = st.checkbox(f"Enable Wavelength {name}", value=True, key=f"{name}_active")
        if act:
            st.markdown(f"**[{name} Channel] Parameters**")
            wl = dual_input("λ Wavelength (nm)", 400.0, 750.0, def_l, 0.01, f"{name}_wl", "%.2f", container=st.sidebar)
            limit_p = wl / get_refractive_index(wl, n_d, V_d)
            icg_p = dual_input("Λ_ICG Period (nm)", 100.0, 1000.0, def_p, 0.01, f"{name}_icg", "%.2f", container=st.sidebar)
            st.caption(f"Physical Limit Pitch: {limit_p:.2f}nm")
            epe_p = dual_input("Λ_EPE Period (nm)", 100.0, 1000.0, icg_p if single_layer_sync else def_p, 0.01, f"{name}_epe", "%.2f", container=st.sidebar) if "Path B" in path_choice else None
            
            if auto_oc_angle:
                oc_p = 300.0 
            else:
                oc_p = dual_input("Λ_OC Period (nm)", 100.0, 1000.0, icg_p if single_layer_sync else def_p, 0.01, f"{name}_oc", "%.2f", container=st.sidebar)
            
            st.markdown("*Diffraction Efficiencies*")
            eff_icg = dual_input("ICG Efficiency", 0.00, 1.00, 0.30, 0.01, f"{name}_efficg", "%.2f", container=st.sidebar)
            eff_epe = dual_input("EPE Efficiency", 0.00, 1.00, 0.20, 0.01, f"{name}_effepe", "%.2f", container=st.sidebar) if "Path B" in path_choice else 1.00
            eff_oc = dual_input("OC Efficiency", 0.00, 1.00, 0.40, 0.01, f"{name}_effoc", "%.2f", container=st.sidebar)
    ...
    wl_R = get_wl_inputs_form("R", 638.0, 300.0, n_d_in, abbe_v_in)
    wl_G = get_wl_inputs_form("G", 520.0, 300.0, n_d_in, abbe_v_in)
    wl_B = get_wl_inputs_form("B", 450.0, 300.0, n_d_in, abbe_v_in)

    st.markdown("---")
    # [핵심 개정 2] 물리 연산을 유발하는 명시적인 배포 '실행' 버튼 설계
    run_simulation_button = st.form_submit_button(label="▶ Run Simulation", use_container_width=True)

# --- 6. Execute Multi-Channel Mathematical Computations (Only when form is submitted) ---
if "sim_calculated_data" not in st.session_state:
    st.session_state["sim_calculated_data"] = None

if run_simulation_button or st.session_state["sim_calculated_data"] is not None:
    if run_simulation_button:
        ...
        st.session_state["sim_calculated_data"] = results
    else:
        results = st.session_state["sim_calculated_data"]
    ...
    st.info("💡 사이드바 패널에서 설정을 세팅한 뒤 하단의 [▶ Run Simulation] 버튼을 클릭하면 광학 연산 및 다이어그램 렌더링이 시작됩니다.")
